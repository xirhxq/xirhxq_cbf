...
void Robot::time_forward(double _t) {
    if (p.len() <= 1 && btry <= 40){
        ...
        double h_val = h(p, btry);
//        printf("h(x, y, btry) = %.6lf\n", h_val);
        double alphah = 0.1 * pow(h_val, 3) ;
//    Point sat_v = v.saturation(s);
        Point sat_v = v;
        Point hxy = Point(hx(p, btry), hy(p, btry));
        if (sat_v * hxy + alphah - 1 >= 0){
            p = p + sat_v * _t;
        }
        else{
            Line l = Line(hxy.x, hxy.y, alphah - 1);
...
//        printf("cbf_v in line = %.2lf\n", hxy.x * cbf_v.x + hxy.y * cbf_v.y + alphah);
